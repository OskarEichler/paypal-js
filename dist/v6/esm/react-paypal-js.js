...
function deepEqual(obj1, obj2, maxDepth = 10, currentDepth = 0) {
  ...
  for (const key of keys1) {
    if (!Object.prototype.hasOwnProperty.call(record2, key) || !deepEqual(record1[key], record2[key], maxDepth, currentDepth + 1)) {
      return false;
    }
  }
  return true;
...
const GooglePayOneTimePaymentButton = ({
  ...
  }, [isHydrated, isPending, createGooglePayButton, buttonType, buttonColor, buttonSizeMode, buttonLocale]);
  // Cleanup on unmount
  useEffect(() => {
    return () => {
      handleDestroy();
